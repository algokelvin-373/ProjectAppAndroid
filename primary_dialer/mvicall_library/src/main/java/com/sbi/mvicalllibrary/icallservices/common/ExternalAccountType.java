...
public class ExternalAccountType extends BaseAccountType {
    ...
    ExternalAccountType(Context context, String packageName, boolean isExtension, XmlResourceParser injectedMetadata) {
        ...
        mInviteActionLabelResId = resolveExternalResId(context, mInviteActionLabelAttribute, syncAdapterPackageName, ATTR_INVITE_CONTACT_ACTION_LABEL);
        mViewGroupLabelResId = resolveExternalResId(context, mViewGroupLabelAttribute, syncAdapterPackageName, ATTR_VIEW_GROUP_ACTION_LABEL);
        titleRes = resolveExternalResId(context, mAccountTypeLabelAttribute, syncAdapterPackageName, ATTR_ACCOUNT_LABEL);
        iconRes = resolveExternalResId(context, mAccountTypeIconAttribute, syncAdapterPackageName, ATTR_ACCOUNT_ICON);
    ...
    public static XmlResourceParser loadContactsXml(Context context, String resPackageName) {
        ...
        @SuppressLint("WrongConstant") final List<ResolveInfo> intentServices = pm.queryIntentServices(intent, PackageManager.GET_SERVICES | PackageManager.GET_META_DATA);
                ...
                for (String metadataName : METADATA_CONTACTS_NAMES) {
                    ...
                    if (parser != null) {
                        if (Log.isLoggable(TAG, Log.DEBUG)) {
                            Log.d(TAG, String.format("Metadata loaded from: %s, %s, %s", serviceInfo.packageName, serviceInfo.name, metadataName));
                        }
                        return parser;
                    }
                }
