...
import com.sbi.mvicalllibrary.icallservices.common.AccountType.EventEditType;
...
public class RawContactModifier {
    ...
    public static ArrayList<EditType> getValidTypes(RawContactDelta state, DataKind kind,
        ...
        if (checkOverall) {
            ...
            validOverall = (kind.typeOverallMax == -1 || overallCount < kind.typeOverallMax);
        }

        for (EditType type : kind.typeList) {
            final boolean validSpecific = (type.specificMax == -1 || typeCount.get(type.rawValue) < type.specificMax);
            final boolean validSecondary = (includeSecondary || !type.secondary);
