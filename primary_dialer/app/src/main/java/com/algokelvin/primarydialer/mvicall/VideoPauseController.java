...
import com.algokelvin.primarydialer.mvicall.Call.State;
...
import com.algokelvin.primarydialer.mvicall.InCallVideoCallCallbackNotifier.SessionModificationListener;
 ...
 */
public class VideoPauseController implements InCallStateListener, IncomingCallListener, SessionModificationListener {
    ...
    private class CallContext {
        ...
        public String toString() {
            return String.format("CallContext {CallId=%s, State=%s, VideoState=%d}", mCall.getId(), mState, mVideoState);
        }
        ...
        private int mState = Call.State.INVALID;
     ...
     * {@link InCallPresenter}.
     *
     * @param inCallPresenter The {@link InCallPresenter}.
     */
     ...
     * internal state.  Called from {@link InCallPresenter}.
     */
    ...
    private void onPrimaryCallChanged(Call call) {
        ...
        if ((isIncomingCall(mPrimaryCallContext) || isDialing(mPrimaryCallContext)) && canVideoPause && !mIsInBackground) {
    ...
    private static boolean canVideoPause(CallContext callContext) {
        return isVideoCall(callContext) && callContext.getState() == State.ACTIVE;
    }
    ...
    private static boolean isIncomingCall(Call call) {
        return call != null && (call.getState() == State.CALL_WAITING || call.getState() == State.INCOMING);
    }
    ...
    private static boolean isDialing(CallContext call) {
        return call != null && State.isDialing(call.getState());
    }
    ...
    private static boolean isHolding(CallContext call) {
        return call != null && call.getState() == State.ONHOLD;
    }
