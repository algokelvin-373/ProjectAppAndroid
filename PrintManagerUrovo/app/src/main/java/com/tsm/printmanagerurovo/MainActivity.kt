...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_print_02.setOnClickListener {
            contentPrint("Roboto", 30)
        }
        btn_print_03.setOnClickListener {
            contentPrint("Roboto", 15)
        }
        btn_print_04.setOnClickListener {
            val bitmap = getPrintBitmap()
    ...
    private fun contentPrint(fontName: String? = null, fontSize: Int? = null) {
        if (fontName != null)
            mFontStylePanel.fontName = fontName
        if (fontSize != null)
            mFontStylePanel.fontSize = fontSize
        val content = """
    ...
    private fun contentPrintThree() {
        ...
        content.add(getPrintBitmap())
        content.add(CustomPrint("", "Default", 15))
        content.add(CustomPrint("Print test 1", "Default", 15))
        content.add(CustomPrint("", "Default", 15))
        content.add(CustomPrint("Print test 2", "Default", 20))
        content.add(CustomPrint("", "Default", 15))
        content.add(CustomPrint("Print test 3", "Default", 25))
        val msg: Message? = mPrintHandler?.obtainMessage(PRINT_CUSTOM)
        msg?.obj = content
        msg?.sendToTarget()
    }
    ...
    private fun doPrintCustom(printerManager: PrinterManager, content: Any) {
        var ret = printerManager.status //Get printer status
        if (ret == PRNSTS_OK) {
            printCustom(printerManager, content)
